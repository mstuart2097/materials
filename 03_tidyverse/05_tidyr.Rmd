...
How much of thic can you do in 10 mins?

<img src="images/tools.jpeg" width=850>
...
box %>% filter(Movie=="3 Hearts") %>% arrange(Days_new) %>% as.data.frame() %>% head(5)
```
...
```{r}
box %>% filter(Days_new < 100) %>% 
  ggplot(aes(x = Days_new, y = Total.Gross))  + 
  geom_line(aes(group = interaction(Movie, Release)), alpha = 0.5) +   
  xlim(c(0,110)) +
  geom_text(aes(x = Days_new+7, label=Movie), 
            hjust = 1,
            data = box %>% filter(Total.Gross > 2.5*10^8, Days_new < 100) %>% 
              group_by(Movie) %>% filter(Days_new == max(Days_new))) 
```
