...
```
ChickPlus <- ChickWeight %>% group_by(Chick) %>% mutate(gain = weight - weight[Time == 0])

ChickPlus %>% ggplot(aes(x = Time, y = gain, group = Chick)) + facet_wrap(~Diet) + geom_line()

ChickError <- ChickPlus %>% filter(Time == 21) %>% group_by(Diet) %>% summarize(
  mean_gain = mean(gain, na.rm=TRUE),
  sd_gain = sd(gain, na.rm=TRUE)/n()
)

ChickPlus %>% filter(Time == 21) %>%
ggplot(aes(x = Diet)) + geom_point(aes(y = gain)) +
geom_errorbar(aes(ymin = mean_gain-1.96*sd_gain, ymax = mean_gain+1.96*sd_gain, colour = Diet), data = ChickError) + 
